...
{
    block_store_t* bs;
    //bitmap occupied in data blocks, block 10
    //inode table is stored in data blocks, occupied by blocks 6-9
} FS_t;

typedef struct inode {
    uint8_t inodeNum;
    uint8_t numberOfHardLinks;
    uint32_t fileSize; //in bytes
    file_t inodeType; 
    time_t lastAccessTime;
    uint16_t directPtr[6];
    uint16_t indirectPtr;
    uint16_t doubleIndirectPtr;
} inode_t;

typedef struct directory_files {
    char filename[127];
    uint8_t inodeNum;
} directoryFiles_t;
///

...
TEST(d_tests, write_file_fill) 
{
	...
	memset(large_eight_five_b_seven + 555 + 888, 0xBB, 1111);
	memset(large_eight_five_b_seven + 555 + 1111 + 888, 0x77, BLOCK_SIZE_BYTES*3 - 888 - 555 - 1111);
	size_t blocks = 2+6+4+1;	// FBM + mystery + inodes
	ASSERT_EQ(fs_create(fs, fnames[0], FS_REGULAR), 0);
	blocks++;	// root directory should have just been created
	...
	// mystery blockstore usage			    6
	// iNodes and iNodesFBM				    5
	// one data block for root dir			1
	// direct inodes	will eat			6
	// indirect inodes	eat	(1+2048)	 2049
	// how many left for dbindirect inodes?
	// 65536 - 2 - 6 - 5 - 1 - 6 - (1+2048) = 63467
	...
	// +1 because dbl indirect block itself gets used
	blocks += 1;
	...
	// This should have taken 60 * 1024 more blocks of data
	//blocks+=60*1024;
	// each block pointed to in the double indirect functions as another indirect.  Should be 30 of those in use
	//blocks+=30;
	// So, the blocks+=2049 should be done 30 times which is the same as 60*1024 + 30
	...
	// According to my investigation, there's 1996 blocks left
	ASSERT_LE(fs_write(fs, fd, giant_data_chunk, BLOCK_SIZE_BYTES * 2048), BLOCK_SIZE_BYTES * 1996);
	blocks += 1996;
...
TEST(g_tests, seek) 
{
	...
	// FS_SEEK 3
	// Largest file would use non-data blocks of:
	//	 2 - FBM
	//	 6 - Mystery stuff from block_store
    //   5 - iNodes and their FBM
	//	 1 - Root dir
    //   6 - Directs
	//	 1 - Indirect block itself
	//	 1 - Double indirect block itself
	//	32 - Blocks that function as indirects, being pointed to by the double indirect block entries
	//       There is room for more of these, but you run out of blocks before you use more then 32 entries
	//       in the double indirect block itself.
	// So, the max use of non-data blocks = 53
	// That leaves 65536 - 53 = 65483 max data blocks
	// Largest file would be 65483 blocks * 4096 bytes/block = 268,218,368
	// Take this number -1 (zero-based)
	// as the max seek position possible.
	position = fs_seek(fs, fd_one, 268218368, FS_SEEK_CUR);
	ASSERT_EQ(position, 65483*BLOCK_SIZE_BYTES-1);
	score++;

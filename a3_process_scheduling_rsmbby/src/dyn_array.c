...
bool dyn_array_stable_sort(dyn_array_t *const dyn_array, int (*const compare)(const void *, const void *)) 
{
    //just changed the above function to the given timsort definition
    if (dyn_array && dyn_array->size && compare) 
    {
        timsort(dyn_array->array, dyn_array->size, dyn_array->data_size, compare);
        return true;
    }
    return false;
}
